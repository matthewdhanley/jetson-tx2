...
//    cv::VideoCapture cap(1);
    ...
    colorMask.hue_max = 255;
    colorMask.hue_min = 0;
    colorMask.sat_max = 255;
    colorMask.sat_min = 0;
    colorMask.val_max = 255;
    colorMask.val_min = 0;
    ...
    bool *d_Mask;
    gpuErrchk(
            cudaMalloc((void **) &d_Mask, sizeof(bool) * pixels) // allocate space on device for centroids
    );
    ...
    int npoints = 20;
    centroid* plot_points = (centroid *) malloc(npoints * sizeof(centroid));
    ...
    // calculate initial stride variables
    int blockStride = BLOCK_SIZE * BLOCK_SIZE;
    int gridStride = numBlocksX * numBlocksY;

    blockStride = next_pow_of_two(blockStride);
    gridStride = next_pow_of_two(gridStride);
    ...
    while(1){
        ...
        switch(keystroke) {
            ...
            case '0' : {
                ...
                if (trackbar){
                    cv::createTrackbar("Hue Min", "Result", &(colorMask.hue_min), 255);
                    cv::createTrackbar("Hue Max", "Result", &(colorMask.hue_max), 255);
                    cv::createTrackbar("Sat Min", "Result", &(colorMask.sat_min), 255);
                    cv::createTrackbar("Sat Max", "Result", &(colorMask.sat_max), 255);
                    cv::createTrackbar("Val Min", "Result", &(colorMask.val_min), 255);
                    cv::createTrackbar("Val Max", "Result", &(colorMask.val_max), 255);
                    cv::createTrackbar("Erodes", "Result", &n_erodes, 15);
                ...
                // execute gpu kernel
                cuda_mask(d_Mask, d_Pin_hsv, width, height, colorMask, numBlocks, numThreads);
                for(int i=0; i<n_erodes; i++) {
                    cuda_erode(d_Mask, width, height, numBlocks, numThreads);
                }
                cuda_gpu_centroid(d_Mask, width, height, blockStride, gridStride, d_c, d_non_zero, numBlocks, numThreads);
                cuda_applyMask(d_Pout, d_Pin_rgb, d_Mask, width, height, numBlocks, numThreads);
                ...
                // catch rollovers
                plot_i = plot_i % npoints;

                plot_points[plot_i] = c;
                for (int i = 0; i<npoints; i++) {
                    cv::circle(frame_in, cv::Point(plot_points[i].x, plot_points[i].y), 5, cv::Scalar(0, 255, 0), -1);
                }
